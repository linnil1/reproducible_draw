export const tw = {
    ...
    results: {
        fetch: {
            ...
            invalidTimePer5s: '時間必須是 *:*:*0 or *:*:*5 (每五秒).',
            invalidTimePer10Min:
...
兩種資料的更新間隔均為 5 秒。  
然而，這些資料並非即時更新，台灣證券交易所通常於 13:40 更新，我們的系統會於 14:00 同步。

### 欄位:
我們將資料合併，欄位順序如下：
1. **指數值欄位**：保留來源的原始順序。
2. **交易量欄位**：保留來源的原始順序，但排除時間欄位（以避免重複）。

### 格式:
最終的 JSON 輸出包含以上指定順序的所有欄位。範例如下：
