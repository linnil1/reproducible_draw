...
export class Stock extends Data {
    ...
    check(date: Date): DataResult {
        ...
        const taiwanDate = DateTime.fromJSDate(date).setZone('Asia/Taipei')
        const taiwanDateStr = parseInt(taiwanDate.toFormat('HHmm'))
        const nowStr = parseInt(now.toFormat('HHmm'))

        // Check if it's a weekday (Monday to Friday)
        if (taiwanDate.isWeekend) {
            return {
                status: Status.FAIL,
        ...
        if (taiwanDate.day == now.day && nowStr < 1400) {
            // 14:00
            return {
                status: Status.PENDING,
