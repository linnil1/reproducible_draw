<script lang="ts">
    ...
    // Wait for locale to be properly initialized
    waitLocale().then(() => {
        ready = true
    })
...
<div class="fixed right-4 bottom-4 z-50 flex flex-col items-end space-y-2">
    ...
    <div class="relative">
        ...
        {#if languageToggler}
            <ul
                class="absolute right-0 bottom-full mb-2 w-40 overflow-hidden rounded border border-gray-200 bg-white shadow-lg"
            >
